...
if(params$geneID != "") {
  ...
  if(!file.exists(params$fasta_out)) {
    ...
    write.table(c('>reference','ACTG'),params$fasta_out,row.names = F,quote = F,col.names = F)
    ...
    if(candidates$position[i] != '') {variant_annotated$alternate[variant_annotated$position == candidates$position[i]] = paste0("<span style='color: blue;'>",variant_annotated$alternate[variant_annotated$position == candidates$position[i]],"</span>")}
    ...
    write.fasta(sequences = my_sequences, names = names(my_sequences),nbchar = 100, file.out = params$fasta_out, open = "w")
